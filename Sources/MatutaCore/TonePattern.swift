...
/// 내장 알람 사운드스케이프 및 톤 식별자
public enum BuiltInTone: String, CaseIterable, Codable, Sendable {
    case morningHarp = "Morning Harp"
    case warmRhodes = "Warm Rhodes"
    case forestChime = "Forest Chime"
    case acousticSunrise = "Acoustic Sunrise"
    case radar = "Radar"
    case beacon = "Beacon"
    case chime = "Chime"
    case signal = "Signal"
    ...
    public static let `default`: BuiltInTone = .morningHarp
}

/// 알람음의 파형을 코드로 합성한다.
///
/// 오디오 파일 의존성 없이 순수 수학적 파형 합성을 통해 100% 신뢰성을 보장하면서도,
/// 배음(Harmonics)과 아르페지오 엔벨로프를 결합하여 고음질 사운드를 생성한다.
public struct TonePattern: Sendable {
    ...
    // MARK: - 내장 어쿠스틱 사운드스케이프

    public static let morningHarp = TonePattern(
        name: BuiltInTone.morningHarp.rawValue,
        totalDuration: 3.6,
        notes: [
    ...
    public static let warmRhodes = TonePattern(
        name: BuiltInTone.warmRhodes.rawValue,
        totalDuration: 3.2,
        notes: [
    ...
    public static let forestChime = TonePattern(
        name: BuiltInTone.forestChime.rawValue,
        totalDuration: 3.0,
        notes: [
    ...
    public static let acousticSunrise = TonePattern(
        name: BuiltInTone.acousticSunrise.rawValue,
        totalDuration: 2.8,
        notes: [
    ...
    public static let radar = TonePattern(
        name: BuiltInTone.radar.rawValue,
        totalDuration: 1.5,
        notes: [
    ...
    public static let beacon = TonePattern(
        name: BuiltInTone.beacon.rawValue,
        totalDuration: 1.6,
        notes: [
    ...
    public static let chime = TonePattern(
        name: BuiltInTone.chime.rawValue,
        totalDuration: 1.8,
        notes: [
    ...
    public static let signal = TonePattern(
        name: BuiltInTone.signal.rawValue,
        totalDuration: 1.0,
        notes: [
    ...
    public static let ripple = TonePattern(
        name: BuiltInTone.ripple.rawValue,
        totalDuration: 1.8,
        notes: [
    ...
    public static var allBuiltInNames: [String] {
        BuiltInTone.allCases.map(\.rawValue)
    }

    public static func pattern(for tone: BuiltInTone) -> TonePattern {
        switch tone {
        case .morningHarp: return .morningHarp
        case .warmRhodes: return .warmRhodes
        case .forestChime: return .forestChime
        case .acousticSunrise: return .acousticSunrise
    ...
    public static func pattern(named name: String) -> TonePattern {
        if let tone = BuiltInTone(rawValue: name) {
            return pattern(for: tone)
        }
        return pattern(for: .default)
    }
